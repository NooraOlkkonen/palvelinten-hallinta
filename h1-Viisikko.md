...
Googletin neuvoja ja asensin Microsoft Visual C++ 2019 Redistributable Packagen GameTrick-käyttäjän tekemän Youtube-videon ohjeiden avulla. Tämän jälkeen sain asennettua VirtualBoxin onnistuneesti.
...
Esimerkkinä määritin Apache2-palvelun käynnistämisen komennolla ```sudo salt-call --local -l info state.single service.running apache2 enable=true```. Tämä kuitenkin epäonnistui (Result: False / Comment: The named service apache2 is not available / Failed: 1), koska Apache2-palvelua ei ole asennettu virtuaalikoneeseen. 
...
Esimerkkinä komennon ```touch /tmp/foo``` suorittaminen. Käytin komentoa ```sudo salt-call --local -l info state.single cmd.run 'touch /tmp/foo' creates="/tmp/foo"```. Toiminto onnistui (Succeeded: 1).
